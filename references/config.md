...
## Env-first multi-account pattern

For automation that switches GitLab accounts, prefer per-account env files over one shared shell profile. Example:
...
A plain `source ~/.config/gitlab-cli/env/<account>.env` updates the current shell but may leave the values unexported. In that case `glab` can miss the env overrides and silently reuse stored auth from `~/.config/glab-cli/config.yml`.

Use distinct GitLab bot/service accounts when workflows need distinct visible identities. Multiple PATs on one GitLab user still act as that same user.
...
See [commands/config-commands.md](commands/config-commands.md) for full `--help` output.
