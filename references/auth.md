...
If automation must act as different GitLab users, use distinct bot/service accounts. Multiple PATs on one GitLab user help with rotation or scope separation, but they do **not** create distinct visible identities.

Pick the intended GitLab account before the first write (comments, MRs, approvals, API mutations).

A good operational pattern is one env file per account:

```bash
# ~/.config/gitlab-cli/env/reviewer.env
GITLAB_TOKEN=glpat-...
...
That fallback/shared-auth behavior is convenient for humans, but in automated workflows it can cause the wrong GitLab account to post comments, create MRs, or approve work.
...
This assumes the target account env file set `GITLAB_HOST` for the exact GitLab instance you intend to modify. Do not write until both commands clearly show the intended account on that host.
...
6. Repost under the correct account.
7. Verify the thread no longer shows the wrong visible author for the replacement message.

If the wrong-identity write changed state beyond a comment or reply, re-auth as above and then use the matching GitLab reversal for that write under the correct account and host, such as unapproving an MR or issuing the compensating `glab api --hostname "$GITLAB_HOST"` mutation for the exact resource that was changed.
...
- Verify the active account and token path with `glab auth status` and `glab api user` before any GitLab write.
- In shared automation shells, deliberately re-source the intended env file with `set -a; source ...; set +a` before retrying.
...
See [commands/auth-commands.md](commands/auth-commands.md) for detailed flag documentation:
...
**Initial setup:**
- After authentication, see [config.md](config.md) to set CLI defaults
- See [token.md](token.md) for personal/project access token management

**Repository operations:**
- See [repo.md](repo.md) for cloning repositories
